...
public class Sol853_CarFleet {
    ...
    public int carFleet(int target, int[] position, int[] speed) {
        ...
        for (int i = 0; i < position.length; i++) {
            map.put(position[i], (double) (target - position[i]) / speed[i]); // the one closest to the target is the first element in the map
        }
        int res = 0;
        double cur = 0;
        // O(N) One pass for all cars from the end to start (another direction also works).
        for (double time : map.values()) {
            if (time > cur) { // 后面的车用的时间比前面的车时间长，在终点前追不上，形成一个新的FLEET
                res++;
                cur = time;
            }
        }
